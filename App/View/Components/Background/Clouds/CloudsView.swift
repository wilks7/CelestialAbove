...
struct CloudsView: View {
        ...
        TimelineView(.animation) { timeline in
            Canvas { context, size in
                ...
                let resolvedImages = (0..<8).map { i -> GraphicsContext.ResolvedImage in
                    ...
                    resolved.shading = .linearGradient(Gradient(colors: [topTint, bottomTint]), startPoint: .zero, endPoint: CGPoint(x: 0, y: resolved.size.height))
                    return resolved
                }
    ...
    init(thickness: CloudSize.Thickness, topTint: Color, bottomTint: Color) {
...
struct CloudsView_Previews: PreviewProvider {
    static var previews: some View {
        CloudsView(thickness: .regular, topTint: .white, bottomTint: .red)
            .background(.blue)
    }
}
